...
void Sandbox::onInit() {
  ...
  getMainCamera()->setAdaptiveViewport(Engine::get().getWindowSize(),
                                       Engine::get().getWindowSize());
  ...
  auto _squareSpriteRenderer =
      getMainGraph()->addComponent<SpriteRenderer>(square);
  _squareSpriteRenderer->texture =
      TextureAtlasManager::get().getTile("square", "square_0");

  //        auto _text = getMainGraph()->createNode("Text", square);
  //        getMainGraph()->addComponent<TextRenderer>(_text, _font, "Hello
  //        World");
