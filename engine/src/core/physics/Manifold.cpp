...
namespace Physics {
    ...
    void Manifold::Initialize( void )
    {
      ...
      for(uint32 i = 0; i < contact_count; ++i)
      {
        ...
        Physics::Vec2 ra = contacts[i] - A->position;
        Physics::Vec2 rb = contacts[i] - B->position;

        Physics::Vec2 rv = B->velocity + Cross( B->angularVelocity, rb ) -
    ...
    void Manifold::ApplyImpulse( void )
    {
      ...
      for(uint32 i = 0; i < contact_count; ++i)
      {
        ...
        Physics::Vec2 t = rv - (normal * Dot( rv, normal ));
        t.Normalize( );
    ...
    void Manifold::PositionalCorrection( void )
    {
      ...
      Physics::Vec2 correction = (std::max( penetration - k_slop, 0.0f ) / (A->im + B->im)) * normal * percent;
