...
rde_model* rde_rendering_load_obj_model(const char* _obj_path) {
	...
	size_t _mesh_positions_size = _positions_size;
	float* _mesh_positions = (float*)malloc(sizeof(float) * _mesh_positions_size);
	for(size_t _i = 0; _i < _positions_size; _i++) {
		_mesh_positions[_i] = _positions[_i];
	}
	...
	unsigned int* _mesh_indices = (unsigned int*)malloc(sizeof(unsigned int) * _mesh_indices_size);
	unsigned int _indices_pointer = 0;
	for(size_t _i = 0; _i < _faces_size; _i++) {
		obj_face* _face = &_faces[_i];

		if (_face->vertices_count == 3) {
			_mesh_indices[_indices_pointer + 0] = _face->indices[0 * 3];
			_mesh_indices[_indices_pointer + 1] = _face->indices[1 * 3];
			_mesh_indices[_indices_pointer + 2] = _face->indices[2 * 3];
			_indices_pointer += 3;
		} else if(_face->vertices_count == 4) {
			_mesh_indices[_indices_pointer + 0] = _face->indices[0 * 3]; // This gets the 1st element of each group of indices values
			_mesh_indices[_indices_pointer + 1] = _face->indices[1 * 3];
			_mesh_indices[_indices_pointer + 2] = _face->indices[2 * 3];
			_mesh_indices[_indices_pointer + 3] = _face->indices[2 * 3];
			_mesh_indices[_indices_pointer + 4] = _face->indices[3 * 3];
			_mesh_indices[_indices_pointer + 5] = _face->indices[0 * 3];
			_indices_pointer += 6;
		} else {
			int _indices_in_face = _face->vertices_count;
			int _moving_pointer = 1;
			...
			while(_moving_pointer != _indices_in_face - 1) {
				_mesh_indices[_indices_pointer + 0] = _face->indices[0 * 3];
				_mesh_indices[_indices_pointer + 1] = _face->indices[_moving_pointer * 3];
				_mesh_indices[_indices_pointer + 2] = _face->indices[(_moving_pointer + 1) * 3];
				_indices_pointer += 3;
				_moving_pointer++;
			}
		}
	...
	rde_log_color(RDE_LOG_COLOR_GREEN, "	- vertices: %u, indices: %u \n", _mesh_positions_size, _mesh_indices_size);

	rde_mesh* _mesh = rde_struct_create_mesh(_mesh_positions_size / 3, _mesh_indices_size);
