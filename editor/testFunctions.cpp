...
void Editor::onInit() {
	...
	//
	//	   auto _panelNode2 = canvas->graph->createNode("Panel2", _panelNode);
	//	   auto* _panel2 = _panelNode2->addComponent<UIPanel>(UIPanelConfig { .size = {64, 64}, .color = Color::Yellow });
	//	   _panelNode2->getComponent<UIAnchoring>()->setStretch(RDE_UI_STRETCH_FULL_STRETCH);
	// //
	// //
	// //
	...
	////
	//		auto _uiButtonNode2 = canvas->graph->createNode("UIButton2");
	...
	//	   auto _sliderNode2 = canvas->graph->createNode("Slider2");
	//	   auto _slider2 = _sliderNode2->addComponent<UISlider>(UISliderConfig {  });
	//	   _sliderNode2->getTransform()->setPosition(256, 128);
	//
	//        auto _inputTextNode = canvas->graph->createNode("InputText");
	//        auto* _input = _inputTextNode->addComponent<UIInput>(UIInputConfig {  });
	//        _inputTextNode->getTransform()->setPosition(0, 256);
	//
	//        auto _sliderNode = canvas->graph->createNode("Slider");
	...
	//
	//		auto* _textNode2 = canvas->graph->createNode("TextNode2");
	//		_textNode2->addComponent<UIText>(UITextConfig {
	//			.font = engine->manager.fontManager.getFont("MontserratRegular")
	//		});
	//
...
void Editor::onLateUpdate(Delta _dt) {
	...
	if(engine->manager.inputManager.isKeyJustPressed(RDE_KEYBOARD_KEY_A)) {
		editorCamera->setCurrentZoomLevel(editorCamera->getCurrentZoomLevel() + 0.1);
	}

	if(engine->manager.inputManager.isKeyJustPressed(RDE_KEYBOARD_KEY_RIGHT)) {
		auto _pos = editorCamera->node->getTransform()->getPosition();
		editorCamera->node->getTransform()->setPosition(_pos.x + 1, _pos.y);
	}

	if(engine->manager.inputManager.isKeyJustPressed(RDE_KEYBOARD_KEY_LEFT)) {
		auto _pos = editorCamera->node->getTransform()->getPosition();
		editorCamera->node->getTransform()->setPosition(_pos.x - 1, _pos.y);
	}

	if(engine->manager.inputManager.isKeyJustPressed(RDE_KEYBOARD_KEY_UP)) {
		auto _pos = editorCamera->node->getTransform()->getPosition();
		editorCamera->node->getTransform()->setPosition(_pos.x, _pos.y - 1);
	}

	if(engine->manager.inputManager.isKeyJustPressed(RDE_KEYBOARD_KEY_DOWN)) {
		auto _pos = editorCamera->node->getTransform()->getPosition();
		editorCamera->node->getTransform()->setPosition(_pos.x, _pos.y + 1);
	}
...
void Editor::onDebugRender(Delta _dt, RenderManager* _renderManager) {
	...
	//        };
	//
	engine->manager.renderManager.drawPoint(editorCamera->node->getTransform()->getPosition(), Color::White);
